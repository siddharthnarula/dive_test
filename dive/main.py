...
def run_main():
    ...
    parser.add_argument('--inter',help = 'Have a window of interactive file or just save the screenshots at the path provided (0 for not having the window and 1 for the window).',default=1,type=int)
    ...
    for i in range(num_max):
        ...
        if i < len(args.tract) and args.tract[i] is not None:
            ...
            ## Used for color N segments of the bundle along its length {Not tested/implemented for TRX}
            elif (args.streamlines_segmentations) =="Center" or (args.streamlines_segmentations) =="Meta" :
                bundle_caller = Tract(bundle = tract_image.streamlines,tw=args.width_tract)
                ...
                if not hasattr(tract_image, 'groups') or len(tract_image.groups) == 0:
                    ...
                    for index, (group_name, group_indices) in enumerate(tract_image.groups.items()):
                       ...
                       ##HEREEE Check working
                        if args.stats_csv:
                            # print("Herreee",color_map_mask[index])
                            color_map_mask = []
