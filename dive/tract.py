...
from dive.helper import perform_dtw as centroids_MeTA
...
class Tract(Colors_csv):
    ...
    def assignment_map_(self,target_bundle, model_bundle, no_disks,threshold=None,method=None):
        ...
        if method=="Meta":
            centroids = centroids_MeTA(model_bundle=target_bundle,subject_bundle=model_bundle,num_segments=no_disks)
            k = no_disks
    ...
    def tracts_paint(self,method,number_of_streams):
        ...
        if method=="Meta":
            indx = self.assignment_map_(self.bundle,self.bundle,nb_streams,method="Meta")
        indx = np.array(indx)
